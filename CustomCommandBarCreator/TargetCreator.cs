...
namespace CustomCommandBarCreator
{
    public class TargetsCreator
    {
        ...
        public bool WriteTargetsFile(string projectDir,string barFolder,string dsName)
        {
            try
            {
                ...
                if (File.Exists(path))
                    File.Delete(path);
                File.AppendAllText(path, buildProjectTargetString(barFolder,dsName));
                return true;
            }
            catch
            {
        ...
        //Version 2.605
        private string buildProjectTargetString(string barFolder,string dsName)
        {
            ...
            sr.AppendLine("\t\t<Message Text=\"Rename \'.dll\' file extension to \'"+dsName+".CorelAddon\' extension\" />");
            sr.AppendLine("\t\t<Message Text=\"$(CurrentCorelPath)\" />");
            sr.AppendLine("\t\t<Exec Condition=\"Exists('$(TargetDir)"+dsName+ ".CorelAddon')\" Command='del \"$(TargetDir)"+dsName+".CorelAddon\"' />");
            sr.AppendLine("\t\t<Exec Command='rename \"$(TargetPath)\" \""+dsName+".CorelAddon\"'/>");
            sr.AppendLine("\t</Target>");
            sr.AppendLine("\t<Target Name=\"CopyFiles\" AfterTargets=\"Build\">");
            sr.AppendLine("\t\t<Message Text=\"CopyFiles\" />");
            sr.AppendLine("\t\t<Exec Condition=\"Exists('"+ barFolder + "')\" Command='xcopy \"$(ProjectDir)$(OutDir)"+dsName+".CorelAddon\" \"" + barFolder + "\" /y /d /e /c' />");
            sr.AppendLine("\t</Target>");
            sr.Append("</Project>");
            return sr.ToString();
        }
